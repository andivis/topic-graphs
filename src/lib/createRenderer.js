...
export default function createRenderer(progress) {
    ...
    const defaultRectangle = { left: -1000, right: 1000, top: -1000, bottom: 1000 };
    ...
    function onMouseClick(e) {
        ...
        nodeId = (graphBuiltWith == "subreddit") ? trimID : nodeId;
        ...
        (graphBuiltWith == "subreddit") ? window.open('https://www.reddit.com/r/' + nodeId, '_blank') : window.open('https://en.wikipedia.org/wiki/' + nodeId, '_blank');
    ...
    function onBeforeLinkAdded(linkInfo) {
        ...
        let fromId, toId, graphId;
        if (document.getElementById("switchValue").checked == false) {
            fromId = link.fromId.split(' ')[0];
            toId = link.toId.split(' ')[0];
            graphId = graph.rootId;
        }
        else {
            fromId = link.fromId;
            toId = link.toId;
        ...
        if (fromId === graphId || toId === graphId) {

            if (document.getElementById("switchValue").checked == false) {
                fromId = fromId.replace(/[\d/]/gi, '_');
                toId = toId.replace(/[\d/]/gi, '_');
            }
            else {
                fromId = fromId.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\d\s\{\}\[\]\\\/]/gi, '_');
                toId = toId.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\d\s\{\}\[\]\\\/]/gi, '_');
            }

            makeHovered(ui);
            let toUI = nodeContainer.querySelector('#' + toId);
            makeHovered(toUI);

            let fromUI = nodeContainer.querySelector('#' + fromId);
            makeHovered(fromUI);
        }
    }
    ...
    function getNodeIdFromUI(el) {
        while (el) {
            if (el.classList.contains('node')) return el.text();
            el = el.parentNode;
        }
    }
    ...
    function addNode(node) {
        ...
        const rectAttributes = {
            ...
            stroke: '#aaa' // '#58585A' border-color
        };
        ...
        if (document.getElementById("switchValue").checked == true) {
            ...
            if (lastIndex !== -1) {
                trimID = node.id.substring(0, lastIndex);
            }
        }
        ...
        const ui = svg('g', {
            class: 'node',
            // id: node.id,
            id: (graphBuiltWith == "subreddit") ? trimID.replace(/[\d/]/gi, '_') : trimID.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\d\s\{\}\[\]\\\/]/gi, '_'),
